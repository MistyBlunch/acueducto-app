...
const sectionVariants = cva('relative w-full', {
  variants: {
    ...
    background: {
      ...
      lovable:
        'bg-gradient-to-br from-lovable-purple-50 via-white to-lovable-pink-50',
...
const Section = React.forwardRef<HTMLElement, SectionProps>(
  (
    { className, variant, background, overflow, asChild = false, ...props },
    ref,
  ) => {
    ...
    return (
      <Comp
        className={cn(
          sectionVariants({ variant, background, overflow }),
          className,
        )}
        ref={ref}
        {...props}
