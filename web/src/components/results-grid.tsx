...
export function ResultsGrid({ data, isLoading, error, query }: ResultsGridProps) {
  if (isLoading) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-sm text-muted-foreground">
        <div className="flex flex-wrap items-center gap-4">
          ...
          {data.meta.query && (
            <span>para "{data.meta.query}"</span>
          )}
          {data.meta.isPalindrome && (
            <span className="text-success font-medium">
              🎾 Descuento palíndromo aplicado
            </span>
          )}
        </div>
        <span className="text-xs">
          Búsqueda completada en {data.meta.executionTimeMs}ms
        </span>
      </div>
