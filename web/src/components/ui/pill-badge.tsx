  ...
  'inline-flex items-center gap-1.5 rounded-full border px-3 py-1 text-xs font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2',
  {
    variants: {
      variant: {
        ...
        lovable:
          'border-transparent bg-gradient-to-r from-lovable-purple-500 to-lovable-pink-500 text-white shadow-lg hover:shadow-xl hover:scale-105',
        palindrome:
          'border-transparent bg-gradient-to-r from-primary to-secondary text-white font-bold shadow-md hover:shadow-lg animate-glow',
        discount:
          'border-transparent bg-discount text-white font-bold shadow-md hover:shadow-lg hover:bg-discount-dark',
        pulse:
          'border-transparent bg-primary text-primary-foreground animate-pulse shadow',
...
const PillBadge = React.forwardRef<HTMLDivElement, PillBadgeProps>(
  (
    ...
    return (
      <div
        className={cn(
          pillBadgeVariants({ variant, size, interactive }),
          className,
        )}
        ref={ref}
        {...props}
