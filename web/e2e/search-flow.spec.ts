...
test.describe('Product Search Flow', () => {
  ...
  test('should show initial idle state with acueducto theme', async ({ page }) => {
    // Check the header
    await expect(page.getByText('acueductoShop')).toBeVisible()
    await expect(page.getByText('Busca acá todos los productos de nuestra tienda')).toBeVisible()
    ...
    await expect(page.getByText('💡 Tips de búsqueda:')).toBeVisible()
  })

  test('should handle palindrome search with discounts', async ({ page }) => {
    const searchInput = page.getByPlaceholder('Buscar raquetas, pelotas, ropa deportiva...')
    
    // Type a palindrome
    await searchInput.fill('oso')
    
    // Should show palindrome hint
    await expect(page.getByText('🎾 ¡Palíndromo detectado! 50% de descuento aplicado')).toBeVisible()
    
    // Wait for search results
    await expect(page.getByText('2 resultados encontrados')).toBeVisible({ timeout: 5000 })
    
    // Check palindrome discount banner
    await expect(page.getByText('¡Descuento palíndromo activo!')).toBeVisible()
    await expect(page.getByText('🎾 50% Descuento palíndromo')).toBeVisible()
    ...
    await expect(page.getByText('🎾 Descuento palíndromo aplicado')).toBeVisible()
  })

  test('should handle non-palindrome search without discounts', async ({ page }) => {
    const searchInput = page.getByPlaceholder('Buscar raquetas, pelotas, ropa deportiva...')
    ...
    await expect(page.getByText('🎾 ¡Palíndromo detectado!')).not.toBeVisible()
    
    // Wait for search results
    await expect(page.getByText('1 resultado encontrado')).toBeVisible({ timeout: 5000 })
    
    // Should not show palindrome discount banner
    await expect(page.getByText('¡Descuento palíndromo activo!')).not.toBeVisible()
    ...
    await expect(page.getByText('🎾 Descuento palíndromo aplicado')).not.toBeVisible()
  })
  ...
  test('should handle empty search results', async ({ page }) => {
    ...
    await expect(page.getByText('No encontramos productos')).toBeVisible({ timeout: 5000 })
    ...
    await expect(page.getByText('Sugerencias de búsqueda:')).toBeVisible()
  ...
  test('should handle search errors with retry functionality', async ({ page }) => {
    const searchInput = page.getByPlaceholder('Buscar raquetas, pelotas, ropa deportiva...')
    ...
    await expect(page.getByText('Error del servidor')).toBeVisible({ timeout: 5000 })
  ...
  test('should show loading state during search', async ({ page }) => {
    ...
    await expect(page.getByText('Buscando productos...')).toBeVisible({ timeout: 1000 })
  ...
  test('should handle search input debouncing', async ({ page }) => {
    ...
    await expect(page.getByText('🎾 ¡Palíndromo detectado!')).toBeVisible({ timeout: 1000 })
    await expect(page.getByText('2 resultados encontrados')).toBeVisible({ timeout: 5000 })
  })
  ...
  test('should navigate to demo page', async ({ page }) => {
    // Click demo button
    await page.getByText('Ver Demo').click()
    
    // Should navigate to demo page
    await expect(page).toHaveURL('/demo')
    await expect(page.getByText('🎾 Estados de Búsqueda - Demo')).toBeVisible()
  })
  ...
  test('should show product details correctly', async ({ page }) => {
    ...
    await expect(page.getByText('¡50% de descuento palíndromo aplicado!')).toBeVisible()
