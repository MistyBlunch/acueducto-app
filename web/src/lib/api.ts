import { ProductsResponseSchema, SearchParams, ApiErrorSchema, ApiException } from '@/types/api'
    ...
    try {
      ...
      if (!response.ok) {
        ...
        switch (response.status) {
          case 404:
            errorMessage = 'Servicio no encontrado. Verifica que la API esté ejecutándose.'
            break
          case 500:
          ...
          case 503:
          case 504:
            errorMessage = 'Servidor no disponible. Intenta nuevamente en unos momentos.'
            break
          default:
